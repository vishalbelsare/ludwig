...
class MultiNodeCheckpoint(Checkpoint):
    ...
    def load(self, save_path: str, device: torch.device | None = None) -> bool:
        ...
        try:
            ...
            # weights_only=True: the checkpoint path is user-supplied (--model_resume_path,
            # from_checkpoint=True), so treating it as trusted internal data is wrong.
            # Standard optimizer (Adam, SGD) and scheduler state contain only tensors,
            # dicts, lists, and primitives — all allowed by the restricted unpickler.
